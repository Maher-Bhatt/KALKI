...
def deep_scan(url: str, timeout_ms: int = 25000) -> Dict[str, Any]:
    ...
    try:
        ...
        err_msg = str(e)
        if any(k in err_msg.lower() for k in ("executable doesn't exist", "browser", "install")):
            return {"error": "Playwright browser not installed. Run: playwright install chromium", "findings": []}
        return {"error": f"Playwright scan error: {err_msg}", "findings": []}
