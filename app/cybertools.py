...
def dns_lookup(domain: str) -> Dict[str, Any]:
    """Resolve a domain name to its IP address via socket.getaddrinfo."""
    ...
    try:
        ...
        if ip:
            return {"host": domain, "ip": ip}
        return {"host": domain, "error": "no A record found"}
    except socket.gaierror as e:
        return {"host": domain, "error": str(e)}
    except Exception as e:
        return {"host": domain, "error": str(e)}


def public_ip() -> str:
    """Fetch the user's WAN IP address from a public API."""
    for api in ("https://api.ipify.org", "https://checkip.amazonaws.com",
                "https://ifconfig.me/ip"):
        try:
            req = urllib.request.Request(api, headers={"User-Agent": "KALKI"})
...
def ip_info(ip: str = None) -> Dict[str, Any]:
    """Get geolocation and ISP info for an IP (defaults to user's public IP)."""
    target = ip or "json"
...
def http_headers(url: str) -> Dict[str, Any]:
    """Fetch the HTTP response headers for a given URL."""
    url = (url or "").strip()
    if not url:
...
def ping(host: str) -> Dict[str, Any]:
    """ICMP ping a host using the system ping utility."""
    host = (host or "").strip()
    if not host:
        return {"error": "host required"}
    ...
    try:
        ...
        return {"host": host, "output": output, "returncode": result.returncode}
...
# ─────────────────────────────────────────────────────────────
# WIFI RECON (Windows only)
# ─────────────────────────────────────────────────────────────
def wifi_profiles() -> List[str]:
    """List all saved WiFi SSID profiles on this Windows machine."""
    try:
        result = subprocess.run(
...
def wifi_password(ssid: str) -> Dict[str, str]:
    """Extract the saved WiFi password for a given SSID on Windows."""
    ssid = (ssid or "").strip()
    if not ssid:
    ...
    try:
        ...
        return {"ssid": ssid, "error": "password not found (profile may not exist or has no saved key)"}
    except Exception as e:
...
def cve_lookup(cve_id: str) -> Dict[str, Any]:
    """Look up a specific CVE by ID using the NIST NVD API."""
    ...
    try:
        ...
        refs = [r.get("url", "") for r in (cve_data.get("references") or [])[:5]]
        return {
            "cve_id": cve_id,
            "description": desc,
...
def recent_critical_cves(limit: int = 5) -> List[Dict[str, Any]]:
    """Fetch the most recently published critical CVEs from NIST NVD."""
    limit = min(max(1, int(limit)), 20)
    url = ("https://services.nvd.nist.gov/rest/json/cves/2.0"
           "?cvssV3Severity=CRITICAL&resultsPerPage=" + str(limit))
    try:
        req = urllib.request.Request(url, headers={
            "User-Agent": "KALKI-CVE-Feed",
            "Accept": "application/json",
        })
        ...
        for v in vulns[:limit]:
            ...
            score = None
            for vk in ("cvssMetricV31", "cvssMetricV30"):
                ml = metrics.get(vk, [])
                if ml:
...
# ─────────────────────────────────────────────────────────────
def subdomain_enum(domain: str, limit: int = 30) -> Dict[str, Any]:
    """Enumerate subdomains for a domain using Certificate Transparency logs."""
    domain = (domain or "").strip().lower().rstrip(".")
    if not domain:
        return {"error": "domain required"}
    ...
    try:
        ...
        for entry in data:
            ...
            # crt.sh can return wildcard or multi-line entries
            for part in name.split("\n"):
                part = part.strip().lstrip("*.")
                if part and part.endswith("." + domain) and part not in seen:
                    seen.add(part)
                    subs.append(part)
        subs.sort()
        return {
